...
class $$SearchResponse extends Record {}

$$SearchResponse.mappings = {"totalCount":"total_count","incompleteResults":"incomplete_results","items":null}

$$SearchResponse.decode = (_input) => {
  let totalCount = Decoder.field(`total_count`, Decoder.number)(_input)
  if (totalCount instanceof Err) { return totalCount }

  let incompleteResults = Decoder.field(`incomplete_results`, Decoder.boolean)(_input)
  if (incompleteResults instanceof Err) { return incompleteResults }

  let items = Decoder.field(`items`, Decoder.array($$Item.decode))(_input)
  if (items instanceof Err) { return items }

  return new Ok(new $$SearchResponse({
    totalCount: totalCount.value,
    incompleteResults: incompleteResults.value,
    items: items.value
  }))
}
...
$$Owner.mappings = {"login":null,"avatarUrl":"avatar_url","htmlUrl":"html_url"}

$$Owner.decode = (_input) => {
  let login = Decoder.field(`login`, Decoder.string)(_input)
  if (login instanceof Err) { return login }

  let avatarUrl = Decoder.field(`avatar_url`, Decoder.string)(_input)
  if (avatarUrl instanceof Err) { return avatarUrl }

  let htmlUrl = Decoder.field(`html_url`, Decoder.string)(_input)
  if (htmlUrl instanceof Err) { return htmlUrl }

  return new Ok(new $$Owner({
    login: login.value,
...
$$Item.mappings = {"name":null,"fullName":"full_name","owner":null,"htmlUrl":"html_url","description":null,"url":null,"stars":"stargazers_count","updatedAt":"updated_at","tagsUrl":"tags_url"}

$$Item.decode = (_input) => {
  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  let fullName = Decoder.field(`full_name`, Decoder.string)(_input)
  if (fullName instanceof Err) { return fullName }

  let owner = Decoder.field(`owner`, $$Owner.decode)(_input)
  if (owner instanceof Err) { return owner }

  let htmlUrl = Decoder.field(`html_url`, Decoder.string)(_input)
  if (htmlUrl instanceof Err) { return htmlUrl }

  let description = Decoder.field(`description`, Decoder.string)(_input)
  if (description instanceof Err) { return description }

  let url = Decoder.field(`url`, Decoder.string)(_input)
  if (url instanceof Err) { return url }

  let stars = Decoder.field(`stargazers_count`, Decoder.number)(_input)
  if (stars instanceof Err) { return stars }

  let updatedAt = Decoder.field(`updated_at`, Decoder.string)(_input)
  if (updatedAt instanceof Err) { return updatedAt }

  let tagsUrl = Decoder.field(`tags_url`, Decoder.string)(_input)
  if (tagsUrl instanceof Err) { return tagsUrl }

  return new Ok(new $$Item({
    name: name.value,
    ...
    tagsUrl: tagsUrl.value
  }))
}

class $$Tag extends Record {}

$$Tag.mappings = {"name":null,"nodeId":"node_id"}

$$Tag.decode = (_input) => {
  let name = Decoder.field(`name`, Decoder.string)(_input)
  if (name instanceof Err) { return name }

  let nodeId = Decoder.field(`node_id`, Decoder.string)(_input)
  if (nodeId instanceof Err) { return nodeId }

  return new Ok(new $$Tag({
    name: name.value,
    nodeId: nodeId.value
  }))
}
...
scrolls(event) {
  return $Array.do($Array.map(((method) => {
  return method(event)
...
const $Array_Extra = new(class {
  flatMap(func, array) {
    return $Array_Extra.concat($Array.map(func, array))
  }
  ...
  concat(arrays) {
    return $Array_Extra.foldr($Array_Extra.append.bind($Array_Extra), [], arrays)
  }

  foldl(func, initial, array) {
    return array.reduce(func, initial)
  }

  foldr(func, initial, array) {
  ...
  groupsOf(number, array) {
    return groupsOf(array, number)
  }
})
...
const $SearchResults = new (class extends Store {
    constructor() {
    super()
    this.state = {
        repos: $Maybe.nothing(),
  ...
  get searchError() { return this.state.searchError }
  ...
  get searchForRepos() {
    return (async () => {
      ...
      try {
        return await $Http.send($Http.get(`https://api.github.com/search/repositories?q=topic:mint-lang`))
      } catch(_error) {
        let error = _error;
     new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    searchError: `Could not retrieve packages from github`
    })), _resolve)
    })
    ...
    if (_1 instanceof Err) {
     ...
     new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    searchError: `Could not parse json response`
    })), _resolve)
    })
    ...
    let _2 = $$SearchResponse.decode(json)
    ...
    if (_2 instanceof Err) {
     ...
     (async () => {
      ...
      this.setState(_update(this.state, new Record({
    searchError: `could not decode json`
    })), _resolve)
    })
      }
      catch(_error) {
...
$SearchResults.__displayName = `SearchResults`

class $Header extends Component {
  render() {
    return _createElement("nav", {
      "id": `navbar`,
      className: `navbar has-shadow is-spaced is-light`
    }, [_createElement("div", {
      className: `container`
    }, [_createElement("div", {
      ...
      "href": `/`,
      className: `navbar-item` + ` header-brand`
    }, [_createElement($Logo, { "mobileSize": 20, "invert": false, "size": 30 }), _createElement("div", {}, [`MINT Packages`])])])])])
  }
}
...
class $PackagesFound extends Component {
  get totalNumberPackages() {
    return (() => { let _0 = $Maybe.toResult(`No packages found`, this.repos)
    ...
    return $Number.toString(all.totalCount) + ` packages found` })()
  }
  ...
  render() {
    return _createElement("section", {
      "id": `card`,
      className: `section` + ` packages-found-top`
    }, [_createElement("p", {
...
$PackagesFound.displayName = "PackagesFound"

class $Main extends Component {
  get searchForRepos () { return $SearchResults.searchForRepos }

  get repos () { return $SearchResults.repos }

  get searchError () { return $SearchResults.searchError }

  componentWillUnmount () {
    $SearchResults._unsubscribe(this)
  }

  componentDidMount() {
    $SearchResults._subscribe(this)

    return (async () => {
      try {
         await this.searchForRepos
      }
      catch(_error) {
        if (_error instanceof DoError) {
        } else {
  ...
  render() {
    return _createElement("div", {}, [_createElement($Header, {  }), _createElement("div", {}, [this.searchError]), _createElement("section", {
      className: `section`
    }, [_createElement("div", {
      className: `field`
    }, [_createElement("p", {
    ...
    })])])])]), _createElement($PackagesFound, { "repos": this.repos }), _createElement("section", {
      "id": `card`,
      className: `section` + ` main-top`
    }, [_createElement($Repositories, { "repos": this.repos })])])
  }
}
...
class $Repositories extends Component {
  get renderRepos() {
    return (() => { let _0 = $Maybe.toResult(_createElement("span", {}), this.repos)
    ...
    return $Array.map(this.renderSubGroup.bind(this), $Array_Extra.groupsOf(2, all.items)) })()
  }
  ...
  renderSubGroup(group) {
    return _createElement("div", {
      className: `columns`
    }, [$Array.map(this.renderRepo.bind(this), group)])
  }

  renderRepo(item) {
    return _createElement($Repository, { "item": $Maybe.just(item) })
  }
...
class $Repository extends Component {
  ...
  get tagName() {
    return $Array.firstWithDefault(``, $Array.map(((t) => {
    return t.name
    }), this.tags))
  }
  ...
  findTags(url) {
    return (async () => {
     ...
     new Promise((_resolve) => {
      this.setState(_update(this.state, new Record({
    error: `Could not retrieve tags from github`
    })), _resolve)
    })
    ...
    let _2 = Decoder.array($$Tag.decode)(json)
  ...
  render() {
    return (() => { let _0 = $Maybe.toResult(_createElement("span", {}), this.item)
    ...
    return _createElement("div", {
    ...
    }, [i.name, _createElement("span", {
      className: `subtitle is-6 has-text-success` + ` repository-left-pad`
    }, [this.tagName])])])]), _createElement("div", {
      className: `card-content`
    }, [_createElement("div", {
      className: `content`
    }, [i.description])]), _createElement("footer", {
      ...
      className: `repository-left-pad`
    }, [i.owner.login])]), _createElement("p", {
      className: `card-footer-item`
    }, [_createElement("i", {
      className: `fas fa-star`
    }), _createElement("span", {
      className: `repository-left-pad`
    }, [$Number.toString(i.stars)])]), _createElement("p", {
      className: `card-footer-item`
    }, [this.updatedAt.bind(this)(i.updatedAt)])])])]) })()
  }
